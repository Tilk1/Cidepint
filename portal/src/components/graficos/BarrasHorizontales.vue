<template>
  <div>
    ...
    <canvas id="barras_horizontales"></canvas>
  </div>
</template>
...
onMounted(async () => {
  try{
    const response = await new ServicioEstadisticasClase().getInstitucionesMejorTiempoResolucion()
...
function convertir_datos_a_grafico(jsonData) {
  ...
  jsonData.data.forEach(item => {
    DataGrafico[item.institucion] = item.tiempo_promedio;
  });
  return DataGrafico;
}
...
function renderChart() {
  ...
  const graph = document.getElementById('barras_horizontales')
  new Chart(graph, {
    type: 'bar',
    options: {
      indexAxis: 'y',
