<template>
    <div v-if="autenticado()">
        <h1 class="flex items-center text-4xl font-extrabold dark:text-white mb-8">Por favor, complete los datos restantes</h1>
        ...
        <form @submit.prevent="enviarDatos">
            ...
            <button type="submit" value="enviarDatos" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Enviar datos</button>
        </form>
    </div>
</template>
<script setup>
    ...
    const enviarDatos = async() => {
        ...
        const respuesta = await store.completarDatos(datos_usuario);
        if(respuesta.error){
            colorAlerta = "error"
            ...
            colorAlerta = "exito"
            mensajes.value[0] = respuesta.mensajes[0]+`. Se lo redireccionará al inicio en 4 segundos...`;
            mostrarAlerta.value = true;
